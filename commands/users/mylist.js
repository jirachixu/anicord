...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('mylist')
        .setDescription('Gets your anime list.')
    ...
    async execute(interaction) {
        try {
            ...
            fs.readFile('./access_tokens.json', 'utf8', async (err, data) => {
                ...
                const response = await fetch(`https://api.myanimelist.net/v2/users/@me/animelist?fields=list_status&sort=${sort}${status ? `&status=${status}` : ''}&limit=20`, {
                    headers: {
                ...
                for (const result of results) {
                    const embed = new EmbedBuilder()
                        ...
                        .setTimestamp()
                        .setFooter({ text: `My Anime List  •  Page ${i} of ${results.length}` });
                    embeds.push(embed);
                    i++;
                }
