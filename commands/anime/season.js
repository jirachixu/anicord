...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('season')
        .setDescription('Gets the anime airing in the specified season.')
        .addStringOption(option => 
            option.setName('season')
                .setDescription('winter, spring, summer, or fall season')
                .setRequired(true)
                .setChoices(
        ...
        .addIntegerOption(option => 
            option.setName('year')
                .setDescription('the desired year between 1986 and the current year')
                .setRequired(true))
        .addStringOption(option => 
            option.setName('type')
    ...
    async execute(interaction) {
        try {
            ...
            if (!(year <= new Date().getFullYear() && year >= 1986)) {
                await interaction.reply({
                    content: 'Make sure the specified year is in between 1986 and the current year!', 
            ...
            if (!type && !continuing) {
                ...
                response = await fetch(`https://api.jikan.moe/v4/seasons/${year}/${season}?continuing`);
                type = 'all types';
                continuing = 'including continuing'
            } else if (type && !continuing) {
                response = await fetch(`https://api.jikan.moe/v4/seasons/${year}/${season}?filter=${type}`);
                continuing = 'not including continuing'
            } else {
                response = await fetch(`https://api.jikan.moe/v4/seasons/${year}/${season}?filter=${type}&continuing`);
                continuing = 'including continuing'
            }
            ...
            for (const result of results) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setTimestamp()
                    .setFooter({ text: `Anime in ${season} ${year} (${type}, ${continuing})  •  Page ${i} of ${results.length}` });
                embeds.push(embed);
                i++;
            }
